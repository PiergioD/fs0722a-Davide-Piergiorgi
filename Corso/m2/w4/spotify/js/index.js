...
function articleBuonasera(myAlbum) {
  const html = `
    <a  href="../albumPage/album.html?albumId=${myAlbum.id}" class="text-light text-decoration-none col-6 col-md-4" id="a">
    <div class="p-2 " >
    <div class="card">
      <div class="row g-0">
        <div class="col-md-4">
...
function artistiHomepage(artisti) {
  const card = ` <a   href="../artistPage/artisti.html?artistId=${artisti.id}"  class="text-decoration-none col-6 col-md-2  "> <div id="ultimo" class="text-light">
  <div class="card">
    <div class="carta">
    <img
...
// fetch per l'header
const id = 9076739;
loadJSON(`https://striveschool-api.herokuapp.com/api/deezer/album/${id}`).then(id => {
  console.log(id);
  ...
  link.setAttribute('href',`../artistPage/artisti.html?artistId=59`)
...
async function stampaArtista(id) {
  try {
    loadJSON(`https://striveschool-api.herokuapp.com/api/deezer/artist/${id}`).then(data_artist => {    
