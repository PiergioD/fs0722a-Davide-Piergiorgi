...
function listaTracks(data) {
    ...
    <td class="d-none d-md-inline" colspan="2">${convertiInMinuti(data.duration)}</td >
...
window.onload = async () => {
    ...
    loadJSON(`https://striveschool-api.herokuapp.com/api/deezer/album/${AlbumId}`).then(res => {
        console.log(res);
    ...
    for(let play of verde){
        ...
        loadJSON(`https://striveschool-api.herokuapp.com/api/deezer/artist/${res.artist.id}/top?limit=50`).then(res => {
            console.log(res);
            let indexes = [];
            res.data.forEach(element => {
                console.log(element.album.id)
                if (element.album.id != AlbumId && (!indexes.includes(element.album.id))) {
